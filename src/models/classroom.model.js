    ...
    studentProgress: [
      {
        ...
        status: {
          type: String,
          enum: ["not_attempted", "attempted", "completed"],
...
classroomSchema.index({ "inviteTokens.token": 1 });
classroomSchema.index({ "inviteTokens.email": 1, "inviteTokens.status": 1 });
 ...
 */
classroomSchema.methods.createInviteToken = function (
  email,
  createdBy,
  expiresInDays = 7
) {
...
classroomSchema.methods.getPendingInvites = function () {
  ...
  const now = new Date();
  return this.inviteTokens.filter(
    (inv) => inv.status === "pending" && now <= inv.expiresAt
  );
};
...
classroomSchema.methods.addStudent = function (userId) {
  ...
  if (!exists) {
    ...
    this.stats.totalStudents = this.students.filter(
      (s) => s.status === "active"
    ).length;
  } else {
...
classroomSchema.methods.updateStudentProgress = async function(userId, problemShortId, status, score = 0) {
    const progressIndex = this.studentProgress.findIndex(
        p => p.userId.toString() === userId.toString() && p.problemShortId === problemShortId
    );
    ...
    if (progressIndex >= 0) {
        ...
        const existing = this.studentProgress[progressIndex];
        ...
        // Update status (don't downgrade from completed to attempted)
    ...
    await this.save();
    return this.studentProgress[progressIndex >= 0 ? progressIndex : this.studentProgress.length - 1];
};

// Get student progress for a problem
classroomSchema.methods.getStudentProgress = function(userId, problemShortId) {
    return this.studentProgress.find(
        p => p.userId.toString() === userId.toString() && p.problemShortId === problemShortId
    );
};

// Get all progress for a student
classroomSchema.methods.getStudentAllProgress = function(userId) {
    return this.studentProgress.filter(
